...
function loadPage(page) {
    console.log(page)
        //'pages/page' + page + '.blade.php'
    $.ajax({
        url: route('load.page', page)
    }).
...
function addPage(page, book) {
    ...
    console.log(book.turn('hasPage', page))
...
function numberOfViews(book) {
    ...
    return book.turn('pages') / 2;

}

function getViewNumber(book, page) {

    return parseInt((page || book.turn('page')) / 2, 10);
...
$('.submit').click(function() {
    $('.thanks-pop').addClass('active');
    setTimeout(function() {
        $('.thanks-pop').removeClass('active');
    }, 2000)
})